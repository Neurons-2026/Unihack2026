...
using Claude LLM to detect dependency/relationship edges.

Usage:
    python -m test_graph_building.run_graph_llm
    python -m test_graph_building.run_graph_llm --model claude-sonnet-4-6
"""
...
from test_graph_building.graph_builder_llm import (
    detect_edges_with_llm,
    build_graph,
    save_graph,
    print_graph_summary,
)
...
OUTPUT_DIR = Path(__file__).resolve().parent / "output_llm"
...
def main():
    parser = argparse.ArgumentParser(
        description="Build knowledge graph using Claude LLM to detect concept relationships"
    )
    parser.add_argument(
        "--model",
    ...
    parser.add_argument(
        "--output",
        default=None,
        help="Output directory (default: test_graph_building/output_llm)",
    )
    parser.add_argument(
        "--api-key",
        default=None,
    ...
    logger.info("=" * 60)
    logger.info("Knowledge Graph Builder (LLM-based relationship detection)")
    ...
    edges = detect_edges_with_llm(nodes, api_key, model=args.model)
    ...
    try:
        subprocess.run(
            [
                ...
                str(graph_json_path),
                "--title", "AI Concepts — LLM Relationship Graph",
            ],
            check=True,
        )
