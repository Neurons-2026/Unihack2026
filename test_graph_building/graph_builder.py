"""
Knowledge Graph Builder — Builds a concept graph from all_concepts_summary.json.

Nodes = extracted concepts.
Edges = connected when label embeddings (OpenAI text-embedding-3-large) have
...
def load_concepts(summary_path: str | Path) -> list[dict]:
    """Load concepts from all_concepts_summary.json, flattening into a node list."""
    ...
    logger.info(f"Loaded {len(nodes)} concepts from {len(sources)} sources")
    return nodes


def embed_labels(nodes: list[dict], api_key: str) -> list[list[float]]:
    """Embed concept labels using OpenAI text-embedding-3-large."""
    client = OpenAI(api_key=api_key)
    labels = [node["label"] for node in nodes]

    logger.info(f"Embedding {len(labels)} labels with text-embedding-3-large")
    response = client.embeddings.create(input=labels, model="text-embedding-3-large")
