...
def to_content_items(items: list[dict], keywords_map: dict) -> list[dict]:
    ...
    for rank, item in enumerate(items, start=1):
        ...
        output.append(
            {
                ...
                "provenance": {
                    "collector": "anthropic_news_scraper",
                    "collector_version": "0.2.0",
                    "dedupe_key": f"anthropic:{slug}",
                },
        ...
        output[-1]["preprocessing"] = {
            ...
            "quality_notes": ["news_page_scraped", "metadata_cleaned"],
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Scrape Anthropic News into standardized pipeline outputs.")
    ...
    items = scrape_anthropic_news(n=args.limit)
    ...
    content_items_path = out_dir / f"anthropic_news_content_items_{stamp}.json"
    cards_path = out_dir / f"anthropic_news_cards_{stamp}.json"
    latest_content_items_path = out_dir / "anthropic_news_content_items_latest.json"
    latest_cards_path = out_dir / "anthropic_news_cards_latest.json"
    ...
    print(f"Scraped {len(items)} Anthropic articles")
